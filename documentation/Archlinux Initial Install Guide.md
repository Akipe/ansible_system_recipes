...
### Configure terminal
If there is errors about the terminal :
```
...
mkdir /mnt/{boot,home,swap,var,vm,.snapshots} && \
...
mount -t btrfs -o $o_btrfs,subvol=@vm      LABEL=${LABEL_PARTITION_ROOT} /mnt/vm && \
...
mkswap /swap/file && \
swapon /swap/file && \
echo -e "/swap/file none swap defaults 0 0" >> /etc/fstab && \
less /etc/fstab && \
pacman --needed --noconfirm -S \
    wget \
    curl && \
...
useradd \
    ...
    -c $USER_NAME_SHOW \
    $USER_NAME_SHELL && \
echo "Password for ${USER_NAME_SHELL} :" && \
